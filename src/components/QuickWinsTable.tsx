...
import { Zap, TrendingUp, Flag, Clock } from 'lucide-react';
      ...
      <div className="overflow-x-auto">
        {quickWins && quickWins.length > 0 ? (
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {quickWins.map((win) => (
                <tr key={win.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full border ${getEffortColor(win.effort)}`}>
                      {win.effort}
                    </span>
                  </td>
                  <td className="px-6 py-4">
          ...
          <div className="px-6 py-8 text-center text-gray-500">
            <p>No quick wins available at this time.</p>
          </div>
        )}
      </div>
    </div>
